...
export const createSession = (sessionData: { token: string, user: User, csrfToken?: string }): void => {
    const expires = Date.now() + SESSION_DURATION;
    localStorage.setItem('YC_SESSION', JSON.stringify({ ...sessionData, expires }));
};

export const getSession = (): { token: string, user: User, expires: number, csrfToken?: string } | null => {
...
export const signup = async (name: string, email: string, password: string, username: string): Promise<User> => {
    const { user, token, csrfToken } = await apiFetch('/signup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, email, password, username }),
    });
    createSession({ user, token, csrfToken });
    return user;
};

export const login = async (email: string, password: string): Promise<User> => {
    const { user, token, csrfToken } = await apiFetch('/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export const updateUserProfile = async (updates: Partial<Pick<User, 'name' | 'username' | 'profilePic'>>): Promise<User> => {
    ...
    const { user, token, csrfToken } = await apiFetch('/profile', {
        method: 'PUT',
        headers: { 
...
export const resetPassword = async (data: { currentPassword: string; newPassword: string }): Promise<void> => {
    const session = getSession();
    if (!session) throw new Error('Please log in to reset your password.');
