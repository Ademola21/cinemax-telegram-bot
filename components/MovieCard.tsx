...
const MovieCard: React.FC<MovieCardProps> = ({ movie, animationDelay }) => {
  ...
  const style = animationDelay ? { animationDelay, willChange: 'transform, box-shadow' } : { willChange: 'transform, box-shadow' };
  return (
    <Link 
      to={`/movie/${movie.id}`} 
        ...
        <div className="aspect-[3/4] w-full bg-gray-800">
          <img 
            ...
            onError={(e) => {
              ...
              // Try fallback to hqdefault if maxresdefault failed
              if (currentSrc.includes('maxresdefault.jpg')) {
                const videoIdMatch = currentSrc.match(/\/vi\/([^\/]+)\//);
                if (videoIdMatch) {
                  const videoId = videoIdMatch[1];
                  const fallbackUrl = `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`;
                  console.log(`Thumbnail fallback: ${currentSrc} -> ${fallbackUrl}`);
                  img.src = fallbackUrl;
                  return;
                }
              }
              
              // Final fallback to placeholder if hqdefault also fails
              img.style.display = 'none';
              img.parentElement?.classList.add('bg-gray-700', 'flex', 'items-center', 'justify-center');
