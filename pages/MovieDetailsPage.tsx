...
const CommentItem: React.FC<{
  ...
  onCommentChange: () => void;
    ...
    const commentUser = { name: comment.reviewer, id: comment.userId, role: comment.reviewer === 'Yoruba Cinemax' ? 'admin' : 'user', profilePic: comment.userProfilePic };
    ...
    const UserAvatar: React.FC<{ user: { name: string, id: string, profilePic?: string } }> = ({ user }) => {
        if (comment.isAI) {
        ...
        if (user.profilePic) {
            return <img src={user.profilePic} alt={user.name} className="w-10 h-10 flex-shrink-0 rounded-full object-cover border-2 border-gray-600" />;
        }
                    ...
                    <div className="bg-gray-800 rounded-lg p-3 border border-gray-700/50">
                        <div className="flex items-center justify-between">
                            <p className="font-bold text-white flex items-center gap-2">
                                {comment.reviewer}
                                {comment.userRole === 'admin' && <span className="text-xs font-bold text-gray-900 bg-yellow-400 px-2 py-0.5 rounded-full shadow">Admin</span>}
                                {comment.userRole === 'moderator' && <span className="text-xs font-bold text-white bg-blue-500 px-2 py-0.5 rounded-full shadow">Moderator</span>}
                            </p>
...
const MovieDetailsPage: React.FC = () => {
  ...
  const relatedMovies = movies.filter(m => m.category === movie.category && m.id !== movie.id).slice(0, 4);
  ...
  return (
    <div>
        ...
        {!isLoadingRecs && aiRecs && aiRecs.length > 0 && (
