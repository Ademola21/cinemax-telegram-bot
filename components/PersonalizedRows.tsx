...
const PersonalizedRows: React.FC = () => {
  ...
  const becauseYouWatched = useMemo(() => {
    ...
    // Smart recommendations: prioritize same genre and category, then by rating
    return movies
      .filter(m => m.id !== lastWatchedMovie.id)
      .map(m => ({
        ...m,
        similarityScore: 
          (m.category === lastWatchedMovie.category ? 50 : 0) +
          (m.genre === lastWatchedMovie.genre ? 30 : 0) +
          (m.rating * 5) +
          (m.stars.some(star => lastWatchedMovie.stars.includes(star)) ? 15 : 0)
      }))
