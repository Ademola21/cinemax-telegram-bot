...
function getStandardQuality(resolution) {
  if (!resolution || resolution === "audio only") return "audio only";
  ...
  else if (height <= 240) return "240p";
  ...
  else if (height <= 2160) return "2160p (4K)";
  else return `${height}p`;
}

function selectBestFormats(formats) {
  // Group formats by standard quality
  const groups = {};
  for (const f of formats) {
    const standardQuality = getStandardQuality(f.resolution);
    if (!groups[standardQuality]) groups[standardQuality] = [];
    groups[standardQuality].push(f);
  }

  const filtered = [];
  
  for (const quality in groups) {
    const resFormats = groups[quality];
      ...
      if (mp4Formats.length === 1) {
        ...
        if (webmFormats.length > 0) {
          ...
          if (webmFormats.length === 1) {
            ...
            // Compare with all formats in this quality to determine if MP4 is lowest or highest
  ...
  exec(`yt-dlp -j "${url}"`, async (err, stdout) => {
    ...
    try {
      ...
      filtered.forEach((f, i) => {
        ...
        const quality = getStandardQuality(f.resolution);
        console.log(`[${i}] ${preference} ${quality} | ${f.ext.toUpperCase()}${audioInfo} | ${sizeMB} MB | id=${f.format_id}`);
      });
