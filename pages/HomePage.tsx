...
const MOVIES_PER_PAGE_MOBILE = 10;
const MOVIES_PER_PAGE_DESKTOP = 12;
...
const HomePage: React.FC = () => {
  ...
  const trendingMovies = useMemo(() => {
    ...
    // Use the trending service which updates hourly and uses real engagement data
    return getTrendingMovies(movies);
  }, [movies]);

  const recentlyAddedMovies = useMemo(() => {
    if (!movies || movies.length === 0) return [];
    // Sort by createdAt (upload time) in descending order and take the 10 most recent
    return [...movies]
  ...
  const moviesPerPage = isMobile ? MOVIES_PER_PAGE_MOBILE : MOVIES_PER_PAGE_DESKTOP;
