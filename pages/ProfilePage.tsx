...
const ProfilePage: React.FC = () => {
  ...
  const handlePasswordReset = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await storage.resetPassword({ currentPassword, newPassword });
      ...
      setPasswordError((err as Error).message || 'Failed to reset password. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto py-8">
      ...
      <div className="mt-8 bg-gray-800/50 border border-gray-700 p-8 rounded-lg">
        <button
          onClick={() => setShowPasswordForm(!showPasswordForm)}
          className="w-full text-left text-xl font-bold text-white flex items-center justify-between"
        >
        ...
        {showPasswordForm && (
          <form onSubmit={handlePasswordReset} className="mt-4 space-y-4">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full px-4 py-2 text-lg font-semibold text-white bg-red-600 rounded-md hover:bg-red-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-red-500 disabled:opacity-50 transition-colors"
            >
